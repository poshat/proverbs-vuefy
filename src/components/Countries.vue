<template>
  <v-container>
    ...
    <v-layout>
      <v-flex xs12>
        <v-list>
          <v-list-tile v-for="item in showCatsLimit" :key="item.id" :to="{name: 'Search', params: {id: item.id, type: 'country', text: item.name}}">
            <v-list-tile-content>
    ...
    computed: {
      ...
      showCats () {
        if (this.search.length > 2) {
          ...
          return this.countries.filter(el => el.name.match(re))
        } else {
