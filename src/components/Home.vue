<template>
  <v-container>
    <v-layout>
      <v-flex xs12>
        <h4>5 случайных поговорок</h4>
      </v-flex>
    </v-layout>

    <v-layout>
      <v-flex xs12>
        <v-card v-for="item in proverbs" :key="item.id" hover>
    ...
    <v-layout>
      <v-flex xs12>
        <v-btn outline block @click.native="getProverbs" class="mt-3">Другие 5</v-btn>
      </v-flex>
    </v-layout>
...
<script>
  export default {
    ...
    methods: {
      getProverbs () {
        this.$store.dispatch('fillRandomProverbs')
      },
      onEdit (id) {
